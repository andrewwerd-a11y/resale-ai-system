...
- ⚪ **Premium intake vision provider toggle** — intake should support selectable providers without changing defaults:
  - `local_ollama` (default, current behavior)
  - `claude_vision` (planned premium option)
  - `openai_vision` / `chatgpt_vision` (future placeholder only, not implemented)
  - Provider selection should include escalation policy and cost guardrails.
  - E2E mock mode must never call paid APIs.
...
- ⚪ **E2E provider-state assertions.** Harness should explicitly report:
  - Ollama intake vision implemented
  - Claude text enrichment implemented
  - Claude vision intake not implemented yet
  - OpenAI/ChatGPT vision not implemented yet
